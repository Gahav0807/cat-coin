...
export default function TopDayPage() {
  const [players, setPlayers] = useState<IPlayerDataTemporary[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    const getPlayers = async () => {
      try {
        const response = await fetch('http://127.0.0.1:9000/getTopDay');
        const data: IPlayerDataTemporary[] = await response.json();
        setPlayers(data);
        setIsLoading(false);
      } catch (error) {
        console.error(error);
    ...
    <div className="main">
      ...
      <div className="header-menu">
        <div className="data-menu">
          <Link className="data-items" href="./day" id='top-day'>День</Link>
          <Link className="data-items" href="./month">Месяц</Link>
